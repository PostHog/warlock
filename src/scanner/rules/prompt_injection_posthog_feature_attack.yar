...
rule prompt_injection_posthog_feature_attack
{
    ...
    strings:
        ...
        $attack_strong_features = /\b(disable|turn\s+off|stop|skip|remove|break|bypass|deactivate|kill|don'?t\s+(enable|use|track))\s+(the\s+)?(session\s+(replay|recording|recordings)|autocapture|auto-capture|automatic\s+event\s+capture|heatmaps?|feature\s+flags?|experiments?|A\/B\s+tests?|A\/B\s+testing|product\s+tours?|group\s+analytics|person\s+profiles?|cohorts?|toolbar)\b/i
        ...
        $attack_dont_track = /\bdon'?t\s+(track|capture|record|log)\s+(this|these|that|the|any|user|page|event)/i
