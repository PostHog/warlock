...
rule prompt_injection_posthog_integration_attack
{
    ...
    strings:
        ...
        $attack_verb_package = /\b(skip|uninstall|remove|delete|disable|avoid|bypass|turn\s+off|break|forget|drop)\s+(the\s+)?posthog(-[a-z_-]+)?\b/i

        // "don't install / don't use / don't add posthog"
        $attack_dont_install = /\bdon'?t\s+(install|use|add|include|import|initialize|setup|set\s+up)\s+(the\s+)?posthog(-[a-z_-]+)?\b/i

        // Verb + "the PostHog (SDK|library|integration|package|tracking|
        //  analytics|client)"
        $attack_generic_target = /\b(skip|uninstall|remove|delete|disable|avoid|bypass|turn\s+off|break|forget|drop)\s+(the\s+)?posthog\s+(sdk|library|integration|package|tracking|analytics|client|instrumentation)\b/i
