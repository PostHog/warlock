...
// doesn't match. The argument gap excludes `)`, so a match never escapes
// the call's own argument list (a propless `.capture('e')` cannot bleed
// into a following call's object).
//
// Known FNs (accepted): top-level PII alongside `$set` in the same call;
// a nested call in the args before the object, e.g. `.capture(fn(), { email })`.
