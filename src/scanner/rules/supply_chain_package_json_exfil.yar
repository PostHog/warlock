...
rule supply_chain_package_json_exfil
{
    ...
    strings:
        ...
        $payload_cred_file_to_net = /"[A-Za-z0-9_:-]+"\s*:\s*"[^"]{0,300}\b(cat|grep|tail|head|awk|sed)\s+[^"]{0,100}(\.env(\.[a-z]+)?|~\/\.(aws|ssh|kube|docker|gnupg)|\.netrc|secrets\.ya?ml|credentials\.json)[^"]{0,100}\|\s*(curl|wget|nc|netcat|socat)/
        ...
        $payload_cred_file_remote = /"[A-Za-z0-9_:-]+"\s*:\s*"[^"]{0,300}\b(curl|wget|scp|rsync)\b[^"]{0,200}(\.env(\.(production|local|staging|dev|development|prod|test))?|~\/\.(aws|ssh|kube|docker|gnupg)|id_rsa|id_ed25519|\.netrc|secrets\.ya?ml|credentials\.json|\.kube\/config|\.docker\/config\.json)/i
