// Catches exfil patterns planted inside a package.json "scripts" value.
// Critical because lifecycle scripts (preinstall/postinstall/prepare)
// run automatically on install.
//
// Known gap: payload-in-a-separate-file indirection (postinstall runs
// node ./foo.js where ./foo.js is the real payload) – diff-review concern.
...
rule supply_chain_package_json_exfil
{
    ...
    strings:
        ...
        // Cred file read piped to network inside a script value.
        ...
        // Cred file pulled via curl/wget/scp/rsync inside a script value.
